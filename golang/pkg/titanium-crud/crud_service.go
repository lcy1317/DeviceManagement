...
import (
	...
	utils "code.corp.bcollie.net/whistle/pkg/titanium-utils"
...
// Service is a precompile contract service for curd.
type Service struct {
	crud         *Crud
...
// AsyncCreateTable async create a new table
...
// Insert entry
func (service *Service) Insert(tableName string, key string, entry *Entry) (*types.Receipt, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("CRUDService Insert failed: %v", err)
	}
	//return parseReturnValue(receipt, "insert")
	return receipt, err
}

// AsyncInsert async insert entry
...
// AsyncUpdate async update entry
...
// GetTableKeySize 通过表名和关键字名获得有多少数据。
// Todo: 这边还没咋改。因为改出问题了
func (service *Service) GetTableKeySize(tableName string, key string) (int, error) {
	if len(key) > tableKeyMaxLength {
		return -1, fmt.Errorf("the value of the table key exceeds the maximum limit( %d )", tableKeyMaxLength)
	}
	opts := &bind.CallOpts{From: service.crudAuth.From}
	...
	result, err := service.crud.Select(opts, tableName, key, string(conditionJSON[:]), "")
	if err != nil {
		return -1, fmt.Errorf("CRUDService Select failed: %v", err)
	}
	// unmarshal result
	var results []map[string]string
	if err := json.Unmarshal([]byte(result), &results); err != nil {
		return -1, fmt.Errorf("CRUDService: Unmarshal the Select result failed: %v", err)
	}
...
func parseReturnValue(receipt *types.Receipt, name string) (int64, error) {
	...
	errorCode, err := utils.BigIntToInt64(bigNum)
	if err != nil {
		return precompiled.DefaultErrorCode, fmt.Errorf("parseReturnValue failed, err: %v", err)
	}
